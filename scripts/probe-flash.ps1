...
# FlashMessage scaffold present on authenticated pages?
$t = $client.GetAsync("$BaseUrl/tasks"); $t.Wait()
$r = $t.Result; $bt = $r.Content.ReadAsStringAsync(); $bt.Wait()
Write-Output ("tasks {0} picker={1} flashScaffold={2}" -f [int]$r.StatusCode, $bt.Result.Contains("Pick an engagement"), $bt.Result.Contains("fixed bottom-4 right-4"))

foreach ($path in @("/timeline", "/kb/new", "/reports", "/settings/account", "/applications")) {
  $t = $client.GetAsync("$BaseUrl$path"); $t.Wait()
  $r = $t.Result; $bt = $r.Content.ReadAsStringAsync(); $bt.Wait()
  $broke = if ($bt.Result -match 'Something broke') { "  <-- BROKE" } else { "" }
  Write-Output ("GET {0} -> {1} len={2}{3}" -f $path, [int]$r.StatusCode, $bt.Result.Length, $broke)
}
