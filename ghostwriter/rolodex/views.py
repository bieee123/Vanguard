...
class ConsultingModeMixin:
    """
    Return 404 for client-management views when multi-client (consulting)
    mode is disabled. Vanguard runs single-company, so the entire client
    management surface is hidden behind :setting:`CONSULTING_MODE`.
    """

    def dispatch(self, request, *args, **kwargs):
        if not settings.CONSULTING_MODE:
            raise Http404
...
def update_project_badges(request, pk):
    ...
    **Template**
    ...
    html = render_to_string(
        ...
        {"project": project_instance, "CONSULTING_MODE": settings.CONSULTING_MODE},
    )
...
class ClientNoteDelete(RoleBasedAccessControlMixin, ConsultingModeMixin, SingleObjectMixin, View):
...
class ClientContactDelete(RoleBasedAccessControlMixin, ConsultingModeMixin, SingleObjectMixin, View):
...
class ClientInviteDelete(RoleBasedAccessControlMixin, ConsultingModeMixin, SingleObjectMixin, View):
...
class AssignProjectContact(RoleBasedAccessControlMixin, ConsultingModeMixin, SingleObjectMixin, View):
...
class ClientLogoDownload(RoleBasedAccessControlMixin, ConsultingModeMixin, SingleObjectMixin, View):
...
class ClientListView(RoleBasedAccessControlMixin, ConsultingModeMixin, ListView):
    """
...
class ClientDetailView(RoleBasedAccessControlMixin, ConsultingModeMixin, DetailView):
    """
...
class ClientExtraFieldJson(ConsultingModeMixin, ExtraFieldJsonView):
    model = Client


class ClientCreate(RoleBasedAccessControlMixin, ConsultingModeMixin, CreateView):
    """
...
class ClientUpdate(RoleBasedAccessControlMixin, ConsultingModeMixin, UpdateView):
    """
...
class ClientDelete(RoleBasedAccessControlMixin, ConsultingModeMixin, DeleteView):
    """
...
class ClientNoteCreate(RoleBasedAccessControlMixin, ConsultingModeMixin, CreateView):
    """
...
class ClientNoteUpdate(RoleBasedAccessControlMixin, ConsultingModeMixin, UpdateView):
    """
...
class ProjectCreate(RoleBasedAccessControlMixin, CreateView):
    ...
    **Template**
    ...
    def setup(self, request, *args, **kwargs):
        ...
        if settings.CONSULTING_MODE and "pk" in self.kwargs:
            pk = self.kwargs.get("pk")
            # Try to get the client from :model:`rolodex.Client`
            if pk:
                self.client = get_object_or_404(Client, pk=self.kwargs.get("pk"))
        elif not settings.CONSULTING_MODE:
            # Vanguard is single-company — always bind projects to the default client
    ...
    def form_valid(self, form):
        ...
        try:
            with transaction.atomic():
                # Save the parent form – will rollback if a child fails validation
                obj = form.save(commit=False)
                if not settings.CONSULTING_MODE and obj.client_id is None:
                    # Single-company mode — guarantee a client is always attached
                    obj.client = get_default_client()
                self.object = obj
                obj.save()
                try:
