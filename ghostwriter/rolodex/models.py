...
from ghostwriter.commandcenter.models import CompanyInformation, validate_endpoint
...
def get_default_client():
    """
    Return the single default :model:`rolodex.Client` used when multi-client
    (consulting) mode is disabled. The ``Project.client`` foreign key is
    required, so Vanguard keeps exactly one client record (the company
    itself) and hides all client management UI behind :setting:`CONSULTING_MODE`.
    """
    client = Client.objects.order_by("id").first()
    if client:
        return client
    company_name = CompanyInformation.get_solo().company_name or "Vanguard"
    return Client.objects.create(name=company_name)
