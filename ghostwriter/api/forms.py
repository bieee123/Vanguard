...
class ServiceTokenForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not settings.CONSULTING_MODE:
            # Vanguard is single-company — drop client-scoped token options
            self.fields["project_scope"].choices = [
                choice
                for choice in self.fields["project_scope"].choices
                if choice[0] != ServiceTokenProjectScope.SELECTED_CLIENTS
            ]
    ...
    def clean(self):
        ...
        if not settings.CONSULTING_MODE and project_scope == ServiceTokenProjectScope.SELECTED_CLIENTS:
            project_scope = ServiceTokenProjectScope.SELECTED
        cleaned_data["project_scope"] = project_scope
        projects = cleaned_data.get("projects")
        clients = cleaned_data.get("clients")
        if "clients" not in cleaned_data:
